...
-- 				reverse_app_types = app_types
		...
		if (luci.http.formvalue("fingerprint") and is_hex(luci.http.formvalue("fingerprint")) and luci.http.formvalue("fingerprint"):len() == 64 and edit_app) then
			resp = luci.sys.exec("echo \"" .. pass_to_shell(signing_msg) .. "\" |SERVALINSTANCE_PATH=/etc/serval serval-sign -s " .. luci.http.formvalue("fingerprint"))
		else
			if (not deleted_uci and edit_app and not uci:delete("applications",UUID)) then
				DIE("Unable to remove old UCI entry")
				return
			end
			resp = luci.sys.exec("echo \"" .. pass_to_shell(signing_msg) .. "\" |SERVALINSTANCE_PATH=/etc/serval serval-sign -s $(SERVALINSTANCE_PATH=/etc/serval servald keyring list |head -1 |grep -o ^[0-9A-F]*)")
		end
